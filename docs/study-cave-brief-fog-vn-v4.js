(function () {
  ...
  function loadState() {
    ...
    saved.sourceMine.started = Boolean(saved.sourceMine.started || safeArray(saved.sourceMine.sources).length || safeArray(saved.sourceMine.quotes).length);
  ...
  function styleBlock() {
    return '<style data-brief-fog-vn-placeholder>' +
      ...
      '.source-mine-room .source-card{width:min(360px,30vw)!important;top:62px!important;left:22px!important;}' +
      ...
      '.source-hotspot-begin{right:8%;top:18%;width:21%;height:31%;border-color:rgba(160,205,255,.82);box-shadow:0 0 28px rgba(83,150,255,.26),inset 0 0 18px rgba(160,205,255,.08);}' +
      '.source-hotspot-desk{left:4%;top:37%;width:31%;height:27%;}.source-hotspot-cart{left:45%;top:39%;width:17%;height:23%;}.source-hotspot-crystals{left:31%;top:13%;width:22%;height:30%;}.source-hotspot-stash{right:4%;bottom:12%;width:16%;height:18%;}.source-hotspot-draft{right:6%;top:35%;width:17%;height:17%;}' +
      '.source-list{margin:10px 0 0;padding-left:18px;}.source-list li{margin-bottom:8px;}.source-mini{font-size:.9rem;opacity:.86;}.evidence-card{border:1px solid rgba(97,70,45,.22);border-radius:12px;padding:10px;margin:8px 0;background:rgba(255,255,255,.48);}.source-locked-note{font-size:.88rem;opacity:.86;margin:.3rem 0 0;}' +
      '@media(max-width:820px){.bf-choice-panel{left:12px;right:12px;top:56px;width:auto;}.bf-girl-marker{right:4%;height:42%;}.bf-fog-mass{width:58%;}.bf-scroll-hotspot{left:14%;bottom:14%;}.source-mine-room .source-card{width:calc(100% - 24px)!important;top:auto!important;bottom:12px!important;}.source-hotspot::after{font-size:.75rem;}.source-hotspot-begin{right:5%;top:15%;width:30%;height:24%;}.source-hotspot-desk{left:4%;top:34%;width:34%;height:22%;}.source-hotspot-cart{left:43%;top:38%;width:22%;height:20%;}.source-hotspot-crystals{left:28%;top:12%;width:28%;height:22%;}.source-hotspot-stash{right:3%;bottom:24%;width:20%;height:14%;}.source-hotspot-draft{right:5%;top:30%;width:24%;height:16%;}}' +
      '</style>';
  }
  ...
  function canMineQuotes(state) {
    return safeArray(state.sourceMine.sources).length > 0;
  }

  function canReviewEvidence(state) {
    return safeArray(state.sourceMine.quotes).length > 0;
  }

  function canCheckGaps(state) {
    return safeArray(state.briefFog.chunks).length > 0 && safeArray(state.sourceMine.quotes).length > 0;
  }

  function canDraftRoute(state) {
    return safeArray(state.sourceMine.quotes).length > 0;
  }

  function sourceHotspots(state) {
    var html = '';
    if (!state.sourceMine.started) {
      return '<button type="button" class="source-hotspot source-hotspot-begin" data-action="source-begin" data-hotspot-label="Begin Source Mine" aria-label="Begin Source Mine"></button>';
    }
    html += '<button type="button" class="source-hotspot source-hotspot-desk" data-action="source-open-notes" data-hotspot-label="Add Source" aria-label="Add Source"></button>';
    if (canMineQuotes(state)) {
      html += '<button type="button" class="source-hotspot source-hotspot-cart" data-action="source-open-quotes" data-hotspot-label="Mine Quote" aria-label="Mine Quote"></button>';
    }
    if (canReviewEvidence(state)) {
      html += '<button type="button" class="source-hotspot source-hotspot-crystals" data-action="source-open-evidence" data-hotspot-label="Evidence Gems" aria-label="Evidence Gems"></button>';
    }
    if (canCheckGaps(state)) {
      html += '<button type="button" class="source-hotspot source-hotspot-stash" data-action="source-open-gaps" data-hotspot-label="Missing Evidence" aria-label="Missing Evidence"></button>';
    }
    if (canDraftRoute(state)) {
      html += '<button type="button" class="source-hotspot source-hotspot-draft" data-action="source-draft-route" data-hotspot-label="Draft Route" aria-label="Draft Route"></button>';
    }
    return html;
  }
  ...
  function sourceUnlockNotes(state) {
    if (!state.sourceMine.started) return '';
    var notes = [];
    if (!canMineQuotes(state)) notes.push('Mine Quote unlocks after you save at least one source.');
    if (!canReviewEvidence(state)) notes.push('Evidence Gems unlock after you save at least one quote, paraphrase, example, or evidence note.');
    if (!canCheckGaps(state)) notes.push('Missing Evidence unlocks when there are Brief Fog chunks and at least one evidence note to compare.');
    if (!canDraftRoute(state)) notes.push('Draft Route unlocks after you have at least one evidence gem.');
    if (!notes.length) return '<p class="source-locked-note">All current Source Mine routes are unlocked.</p>';
    return '<p class="source-locked-note">' + esc(notes.join(' ')) + '</p>';
  }
  ...
  function renderSourceMine(extra) {
    ...
    closePanels();
    node.hidden = false;
    var actions = state.sourceMine.started
      ? '<button type="button" data-action="source-open-notes">Add Source</button>' +
        (canMineQuotes(state) ? '<button type="button" data-action="source-open-quotes">Mine Quote</button>' : '') +
        (canReviewEvidence(state) ? '<button type="button" data-action="source-open-evidence">Evidence Gems</button>' : '') +
        (canCheckGaps(state) ? '<button type="button" data-action="source-open-gaps">Missing Evidence</button>' : '') +
        (canDraftRoute(state) ? '<button type="button" data-action="source-draft-route">Draft Route</button>' : '') +
        '<button type="button" data-action="return-cave-base">Cave Base</button><button type="button" data-action="open-task-map">Task Map</button>'
      : '<button type="button" data-action="source-begin">Begin Source Mine</button><button type="button" data-action="return-cave-base">Cave Base</button><button type="button" data-action="open-task-map">Task Map</button>';
    var intro = state.sourceMine.started
      ? '<p>Gather source ore, mine evidence gems, polish them in your own words, then link them to Brief Fog chunks.</p>'
      : '<p>Source Mine is a gather quest. Begin at the glowing tunnel, then collect source ore and evidence gems before drafting.</p>';
    node.innerHTML = styleBlock() + '<section class="simple-room source-mine-room"><p class="scene-label">Source Mine</p>' + sourceHotspots(state) + '<article class="stage-card simple-card source-card"><h2>Source Mine</h2>' + intro + '<p><strong>Sources:</strong> ' + sources.length + ' · <strong>Evidence gems:</strong> ' + quotes.length + '</p>' + saveInfo(state) + '<div class="simple-actions">' + actions + '</div>' + sourceUnlockNotes(state) + '</article>' + (extra || "") + '</section>';
  }
  ...
  function saveQuote() {
    ...
    state.current = "source-mine";
    saveState(state, "Evidence gem saved; Evidence Nodes unlocked");
    openQuotes();
  }

  function openEvidence() {
    var state = loadState();
    if (!canReviewEvidence(state)) return renderSourceMine(sourcePanel("Evidence Gems", '<p>No evidence gems yet. Save a source, then mine a quote or paraphrase.</p>' + saveInfo(state)));
    var chunks = safeArray(state.briefFog.chunks);
    var quotes = safeArray(state.sourceMine.quotes);
    ...
    body += '<div class="simple-actions"><button type="button" data-action="source-open-quotes">Mine quote</button>' + (canCheckGaps(state) ? '<button type="button" data-action="source-open-gaps">Missing evidence</button>' : '') + '<button type="button" data-action="source-draft-route">Draft Route</button></div>';
    renderSourceMine(sourcePanel("Evidence Gems", body));
  }

  function openGaps() {
    var state = loadState();
    if (!canCheckGaps(state)) return renderSourceMine(sourcePanel("Missing Evidence", '<p>Missing Evidence unlocks once there are Brief Fog chunks and at least one evidence gem to compare.</p>' + saveInfo(state)));
    var chunks = safeArray(state.briefFog.chunks);
    var quotes = safeArray(state.sourceMine.quotes);
    ...
    var body = saveInfo(state);
    if (!gaps.length) {
      body += '<p>Every current Brief Fog chunk has at least one linked evidence gem. That does not mean the evidence is final, but the obvious empty gaps are covered.</p>';
    } else {
  ...
  function draftRoute() {
    var state = loadState();
    if (!canDraftRoute(state)) return renderSourceMine(sourcePanel("Draft Route", '<p>Draft Route unlocks after you save at least one evidence gem.</p>' + saveInfo(state)));
    var body = saveInfo(state) + '<p>Draft Route is the next room after Source Mine. For now this checkpoint confirms your source notes are saved and ready to become a paragraph plan later.</p><p><strong>Evidence gems ready:</strong> ' + safeArray(state.sourceMine.quotes).length + '</p><div class="simple-actions"><button type="button" data-action="source-open-evidence">Review evidence gems</button><button type="button" data-action="open-task-map">Task Map</button></div>';
